...
export function Button({ children, variant = 'primary', sz = 'md', onClick, disabled, full, style: extra, type = 'button' }: Props) {
  const { theme: T } = useTheme()
  const [hov, setHov] = useState(false)

  const variants: Record<Variant, CSSProperties> = {
    primary:   { background: T.accent,      color: '#fff',        border: 'none' },
    secondary: { background: T.accentDim,   color: T.accentLight, border: `1.5px solid rgba(155,135,245,0.25)` },
