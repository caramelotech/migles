...
export function EventCard({ event }: { event: Event }) {
  ...
  return (
    <Card onClick={() => navigate(`/events/${event.id}`)} style={{ display: 'flex', gap: 14, alignItems: 'flex-start' }}>
      <DateChip dateLabel={event.dateLabel} color={cc} />
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 10, marginBottom: 5 }}>
          <div style={{ fontFamily: 'DM Serif Display, serif', fontSize: 17, lineHeight: 1.25, color: T.text }}>
            {event.title}
          </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <div style={{ marginLeft: 'auto', fontSize: 11, color: cc + 'cc', background: cc + '14', padding: '2px 8px', borderRadius: 20, fontWeight: 500 }}>
